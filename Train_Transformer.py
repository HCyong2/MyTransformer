...
model_dir = os.path.join('./models', f"Resnet_CutMix_{run_time}.pth")
# 是否保存模型
save = 1
...
def train_and_test(net, trainloader, testloader, criterion, optimizer, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            inputs, labels = CutMix.cutmix(inputs, labels)
            real_labels, mix_labels, lam = labels
            # 计算梯度
            optimizer.zero_grad()
            outputs = net(inputs)
            # 计算损失
            loss = CutMix.cutmix_criterion(outputs, labels, criterion)
            loss.backward()
            optimizer.step()
            # 计算并保存训练数据
            running_loss += loss.item()
            _, predicted = torch.max(outputs.data, 1)
            # 这里的准确率计算修改成了CutMix后的混合准确率
            correct += lam * (predicted == real_labels).sum().item() + (1 - lam) * (
                    predicted == mix_labels).sum().item()
...
if __name__ == '__main__':
    ...
    model_load_dir = r".\models\RN_2024_06_19_13_28.pth"
